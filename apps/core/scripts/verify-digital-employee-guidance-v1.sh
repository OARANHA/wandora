...
SUFFIX="$$"
NET="wandora-employee-guidance-v1-net-$SUFFIX"
DB="wandora-employee-guidance-v1-db-$SUFFIX"
DB_NAME="wandora_employee_guidance_test"
DB_PASSWORD="wandora-employee-guidance-test-only"
CORE_PASSWORD="wandora-employee-guidance-core-test-only"
FIXTURE_PASSWORD="wandora-employee-guidance-fixture-test-only"
...
verifier="VERIFY_20260925_DIGITAL_EMPLOYEE_GUIDANCE_CONTRACT_V1.sql"
...
docker run --rm --network "$NET" -v "$ROOT/apps/core:/src:ro"   -e DATABASE_URL="postgresql://wandora_core_runtime:${CORE_PASSWORD}@${DB}:5432/${DB_NAME}"   -e FIXTURE_DATABASE_URL="postgresql://wandora_fixture_admin_test:${FIXTURE_PASSWORD}@${DB}:5432/${DB_NAME}"   "$NODE_IMAGE" sh -lc   'mkdir -p /app &&
   ...
   npm run typecheck &&
   node --import tsx --test --test-concurrency=1      test/human-digital-employee-guidance-route.test.ts      test/employee-guidance-runtime-contract.test.ts      test/employee-guidance-runtime-projection.integration.test.ts      test/paperclip-execution-readiness.test.ts      test/paperclip-execution-service.integration.test.ts'

echo "DIGITAL_EMPLOYEE_GUIDANCE_V1_VERIFY_OK"
