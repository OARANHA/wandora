...
- ADR 0083 closes the Global Runtime Gate Activation Preflight V1: the exact gate delta is one canonical Core environment overlay; zero eligibility rows guarantee no new tenant availability; global-gate-first is selected before any tenant eligibility; execution remains separate.
