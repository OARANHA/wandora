...
- ADR 0109 refreshes that recovery prerequisite with one fresh official Paperclip backup paired out of the Docker volume with the exact current `master.key`; disposable PG18 restore proves the live canary secret/config, MEDICSPRO company, matching-key decrypt/hash and wrong-key rejection.
- ADR 0110 closes the MEDICSPRO Organization Adapter custody/config/binding execution by state-first reconciliation after live state advanced beyond ADR 0109: exactly one operator-owned Wandora binding, one deterministic protected HMAC, one company-owned active `local_encrypted` secret and one exact company-scoped `secret_ref` config are live with hash equality and healthy plugin state. MEDICSPRO remains at zero eligibility/employee/hire effect, Human Send and Gateway outbound remain OFF, and no duplicate effect was replayed. The ADR 0109 pair is now a valid pre-wiring snapshot but not a current post-wiring DR pair; refresh it before tenant eligibility.
