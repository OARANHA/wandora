...
- ADR 0077 freezes the first customer-like hire proof: reuse the exact reviewed PR #109 candidate artifact in a no-ingress private Core, create a separate temporary Supabase session for the real canary owner, execute one paused-first hire, prove same-key and same-catalog dedupe, then revoke only that temporary session and remove the candidate.
