...
- ADR 0117 promotes the Paperclip -> Wandora/Mastra bridge into code/CI only: a canonical `wandora_mastra` external adapter, dedicated file-backed HMAC, opaque run-token identity check through Paperclip, active-Wandora-employee binding enforcement and AgentRuntime/Mastra reuse. Migration 014/adapter/Core bridge remain not live until separate activation review.
