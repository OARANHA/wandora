...
- ADR 0104 executes that exact V2 request: MEDICSPRO is live as one active employee-free tenant with one active owner and one V2 idempotency row; provider bindings, hire state and eligibility remain unchanged/zero for MEDICSPRO. A genuine normal owner `/api/v1/me` read remains the next separate no-mutation validation rather than using privileged impersonation.
