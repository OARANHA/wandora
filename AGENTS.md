...
- ADR 0095 closes Customer Owner First Real Access End-to-End Validation Preflight V1 without external effect: owner-access Web and the recovery edge are live, but Auth still points to unresolved development/default SMTP `supabase-mail:2500` and no real new customer owner/organization target exists; first real invite is blocked until transactional e-mail delivery and a genuine target are separately proven.
- ADR 0096 closes Customer Owner Transactional E-mail Delivery Foundation Preflight V1 without provider wiring or e-mail: Resend SMTP is selected behind Supabase Auth using `notify.wandora.com.br`, São Paulo sending region, a sending-only domain-restricted key and Docker-secret custody; live GoTrue remains unchanged until a separate provider/domain/credential execution and later Auth activation.
- ADR 0097 completes the Resend sender-domain + credential provisioning execution: `notify.wandora.com.br` is verified in `sa-east-1`, provider DKIM/sending CNAMEs and subdomain-specific DMARC `p=none` are live, and the dedicated SMTP credential is held only at `/opt/wandora/data/supabase/secrets/gotrue_smtp_pass` under `root:wandora-ops/0640`. SMTP AUTH+QUIT is proven without MAIL/RCPT/DATA. Live GoTrue remains on the old relay; do not treat provider readiness as Auth activation.
- ADR 0098 closes GoTrue SMTP Activation Preflight V1 without production activation: the versioned Auth overlay now resets the inherited SMTP password, mounts only the protected secret, reads it through a minimal root startup wrapper and immediately drops PID 1 back to the pinned image `supabase` UID/GID 1000. Local Compose file secrets cannot remap uid/gid/mode, so do not weaken `root:wandora-ops/0640` or hard-code the host group GID. Activation remains a separate one-service `auth` recreation and must not send mail.
- ADR 0099 executes that activation in production exactly once: only `supabase-auth` is recreated, GoTrue is healthy on Resend SMTP `smtp.resend.com:587` with `Wandora <acesso@notify.wandora.com.br>`, the file-backed credential remains absent from Compose/`.env`/container `Config.Env`, final PID 1 is UID 1000, STARTTLS is proven from the recreated Auth namespace, and no invite/recovery/test mail or customer state is created.
- ADR 0100 closes Customer Owner First Real Invite Execution Preflight V1 as a controlled NO-GO: the exact service-role `POST /auth/v1/invite` contract, redirect, collision gates, ambiguity reconciliation and pre-accept revocation boundary are frozen, but no genuine new owner e-mail has been explicitly authorized. Do not guess/scrape a target, reuse proof/legacy state, or retry an uncertain invite; execution remains blocked until the operator/user supplies the intended owner e-mail.
- ADR 0101 executes exactly one real owner invite after explicit target authorization: the target had zero Auth collision, the corrected Supabase identity mapping guard passed, the invite returned HTTP 200, and read-back proves one pending unconfirmed invited Auth user plus one one-time token. No retry, tenant provisioning, eligibility or downstream provider effect occurred; next is invite acceptance + first-password validation.
- ADR 0102 accepts the first real owner invite/access validation: invite consumption, first password and a later fresh password grant are proven; `/api/v1/me` then fails closed as `unlinked` because the Auth subject has no Wandora identity or membership yet. Do not bypass this boundary with direct inserts; use Private Tenant Provisioning V2 in a separate reviewed slice.
- ADR 0103 closes the first real tenant provisioning preflight: the exact MEDICSPRO V2 request, deterministic slug/request key, hashed Auth-subject gate, least-privilege execution identity, replay rules and postconditions are frozen.
- ADR 0104 executes that exact V2 request: MEDICSPRO is live as one active employee-free tenant with one active owner and one V2 idempotency row; provider bindings, hire state and eligibility remain unchanged/zero for MEDICSPRO.
- ADR 0105 proves the genuine normal owner access path without impersonation: fresh login `/api/v1/me` returns 200, MEDICSPRO renders as the customer organization, and tenant-authorized Trabalho/Equipe/Conversas reads return 200 while MEDICSPRO remains employee-free, provider-unwired and ineligible for hire.
- ADR 0106 closes the MEDICSPRO Paperclip company-bootstrap preflight without provider mutation: reuse the proven official one-shot instance-admin CLI path with explicit private `--api-base`, frozen two-company baseline and stop-on-ambiguity reconciliation; company creation remains separate from HMAC/config/binding, eligibility and employee hire.
- ADR 0107 executes exactly one MEDICSPRO Paperclip company create through that one-shot path: provider company `a63f27a8-dbac-4552-a456-b3a21302226b` is active with owner membership, zero agents/secrets/plugin config, while Wandora binding, eligibility and employee/hire state remain zero.
- ADR 0108 freezes the MEDICSPRO Organization Adapter custody/config/binding contract but blocks wiring execution until a fresh out-of-volume Paperclip DB + current `master.key` recovery pair represents the current state; the retained ADR 0075 DB predates the live canary secret and the MEDICSPRO provider company.
- ADR 0109 refreshes that recovery prerequisite with one fresh official Paperclip backup paired out of the Docker volume with the exact current `master.key`; disposable PG18 restore proves the live canary secret/config, MEDICSPRO company, matching-key decrypt/hash and wrong-key rejection. MEDICSPRO wiring remains the next separate effect.
- ADR 0110 makes MEDICSPRO Organization Adapter wiring live: exact operator binding, deterministic Core HMAC custody, one company-owned `local_encrypted` secret and company-scoped `secret_ref` config are green; the adversarial review rotated the secret to an exact `crypto.randomBytes(32)` value before closure, while eligibility/employee/hire/outbound remain zero/off.
- ADR 0111 accepts MEDICSPRO as the first real `ana-commercial-v1` eligibility target after revalidating genuine owner access, zero employee/hire collisions and exact Organization Adapter wiring; eligibility remains zero until the separate serialized operator execution.
- ADR 0112 makes exactly one first-real-tenant eligibility live: MEDICSPRO + `ana-commercial-v1`; serialized operator execution committed cleanly while MEDICSPRO employees/hire/provider-agent state remained zero and Human Send/Gateway outbound remained OFF.
- ADR 0113 historically isolated the repository-scoped self-hosted GitHub Actions runner from production Docker/sudo; ADR 0158 supersedes it for normal CI, which now uses GitHub-hosted `ubuntu-24.04` runners.
- ADR 0114 freezes the first real MEDICSPRO owner hire boundary: normal owner browser only, one retained idempotency key, no activation/outbound.
- ADR 0115 makes exactly one MEDICSPRO Ana live through that normal owner hire flow as `paused + supervised`, with one provider binding and one completed hire operation; Paperclip agrees on one paused managed Ana.
- ADR 0116 closes the first real activation preflight as NO-GO: `wandora_mastra` is not registered live and the live Organization Adapter does not hold `agents.resume`; Ana remains paused and outbound stays OFF.
- ADR 0117 is merged as the code/CI-only Paperclip -> Wandora/Mastra bridge foundation: canonical `wandora_mastra` external adapter, dedicated file-backed HMAC, opaque run-token identity check through Paperclip, active-Wandora-employee binding enforcement and AgentRuntime/Mastra reuse. PR #166 is canonical `main` at `7bc8c4790e37b0410703bf58979458200810d5a9`; migration 014/adapter/Core bridge remain not live until separate activation review.
- ADR 0118 closes Production Execution Bridge Activation Preflight V1 as NO-GO for live activation: migration 014/adapter/Core bridge remain absent/off; exact adapter/Core artifacts and future execution order are frozen, but canonical Paperclip bridge runtime custody, bridge-specific Core readiness and one disposable real-run-token Paperclip -> Core -> Mastra attestation must be implemented/proven first. `agents.resume`, Ana activation, Human Send and Gateway outbound remain separate and OFF.
