...
2. define **Supervised Proposal Review V1** as the smallest Wandora-owned customer-facing contract for `attention-required` work, proposal context and human supervision;
3. expose tenant-authorized Core read APIs to Wandora Web without exposing `wandora_private.inbound_event_receipts`, provider metadata or Mastra internals;
4. prove cross-tenant denial and inactive/disabled human denial before connecting the browser to canonical state;
