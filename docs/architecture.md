...
5. ADR 0085 proves no current active tenant qualifies for the first new catalog eligibility; next is Clean Tenant Rollout Candidate Preparation Preflight V1, and eligibility must not be forced onto the completed-hire canary/proof tenants or the legacy `Empresa Exemplo` tenant;
