...
5. ADR 0095 closes the first real owner-access preflight without effect. ADR 0096 selects Resend SMTP through the existing Supabase Auth boundary. ADR 0097 makes the provider-side foundation live. ADR 0098 now freezes the exact Auth-only activation candidate: protected file secret, inherited password reset, minimal startup read followed by immediate drop to the pinned `supabase` UID/GID, byte-identifiable rollback, and `--no-deps --force-recreate auth` as the only service recreation. Live GoTrue still targets `supabase-mail:2500`; the next slice is GoTrue SMTP Activation Execution V1, still without invite/recovery/test mail;
