...
5. ADR 0095 closes the first real owner-access preflight without effect. ADR 0096 selects Resend SMTP through the existing Supabase Auth boundary. ADR 0097 now makes the provider-side foundation live: `notify.wandora.com.br` is verified in `sa-east-1`, DKIM/sending CNAMEs and subdomain-specific DMARC are published, and the dedicated sending credential is in reviewed host custody with SMTP AUTH proven without a message transaction. Live GoTrue still targets `supabase-mail:2500`; the next slice is a no-effect GoTrue SMTP activation preflight before any Auth service recreation or real invite;
