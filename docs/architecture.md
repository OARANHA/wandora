...
5. ADR 0087 implements the normal owner invite/first-password path in code/CI only; next is Customer Owner Interrupted Invite Recovery Contract Preflight V1, because a one-time invite consumed before browser-session loss must have a reviewed Supabase Auth recovery path before real customer onboarding;
