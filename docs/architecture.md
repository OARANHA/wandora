...
Tenant provisioning itself remains provider-free: ADR 0104 did not create Paperclip/control-plane state. Subsequent ADRs 0107–0110 now establish the separate Paperclip company and Organization Adapter wiring for MEDICSPRO, while employee bindings, customer-hire eligibility, hire state and messaging effects remain separate later boundaries.
...
ADR 0110 closes the separately reviewed wiring execution by state-first reconciliation rather than replay. Production had already advanced after the ADR 0109 checkpoint in the frozen order: exact operator-owned Wandora binding → deterministic protected Core HMAC → one company-owned Paperclip `local_encrypted` secret → company-scoped Organization Adapter `secret_ref` config last. Independent hash-only validation proves the Core HMAC equals Paperclip version-1 `value_sha256`/fingerprint; secret usage is exactly one; the plugin is ready/healthy; MEDICSPRO has zero agents/employees/employee-bindings/hire-operations/eligibility. Human Send and Gateway outbound remain OFF.

The ADR 0109 protected pair is consequently a correct pre-wiring snapshot, not a complete current-state DR pair for the newly added MEDICSPRO secret/config. A newer logical backup exists inside the Paperclip volume, but the next separate boundary must refresh and restore-prove the protected out-of-volume DB + current `master.key` pair before MEDICSPRO tenant eligibility is considered.
