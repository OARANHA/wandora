...
Post-upgrade acceptance used only the existing **Wandora Internal Supervised Proof** identity. The proof ran through Paperclip's normal heartbeat service so Paperclip minted the run-scoped JWT internally; the bounded on-demand run and one timer heartbeat that fired during the brief synthetic idle window both completed `succeeded`. The proof agent was then returned to `paused`, its synthetic issue was `cancelled`, pending proof runs were zero, and the proof tenant outbound-attempt count stayed unchanged at 4. A syntactically valid forged JWT returned 401. The local-encrypted Organization Adapter path was proven by resolving the secret before deliberately rejecting an invalid signature. No MEDICSPRO employee or outbound effect was triggered.
