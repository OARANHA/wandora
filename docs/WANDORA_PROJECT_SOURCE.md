...
The minimum Wandora-owned gap is a provider-neutral semantic projection that keeps three layers separate:

- capabilities a business-system integration supports;
- capabilities operationally available to the organization;
- capabilities authorized for the current run.

The final Fast Read capability set is the intersection of organization availability with run-scoped authorization; runtime authorization can only narrow, never expand, semantic availability.
