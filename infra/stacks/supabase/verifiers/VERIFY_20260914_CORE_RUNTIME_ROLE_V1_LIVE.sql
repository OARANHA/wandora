...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_authid WHERE oid = runtime_oid AND rolpassword IS NOT NULL) THEN
    RAISE EXCEPTION 'wandora_core_runtime unexpectedly has a password';
  END IF;
  IF EXISTS (
...
SELECT 'CORE_RUNTIME_ROLE_V1_LIVE_OK' AS verifier;
ROLLBACK;
