...
The SMTP credential exists only at `/opt/wandora/data/supabase/secrets/gotrue_smtp_pass` with `root:wandora-ops/0640` custody. It must not live in this repository, the Supabase `.env`, Compose text, browser-visible configuration or another service environment. ADR 0098 versions and proves the reviewed Auth secret-file startup/rollback path; ADR 0099 executes it in production. Because local Compose file secrets preserve host permissions and ignore `uid/gid/mode`, the wrapper reads the protected file during a minimal root startup window and immediately `exec su` drops GoTrue back to the image `supabase` UID/GID 1000. **Live GoTrue SMTP is now active** on `smtp.resend.com:587` using `Wandora <acesso@notify.wandora.com.br>`; only `supabase-auth` was recreated, STARTTLS was proven from the recreated Auth namespace, and no invite/recovery/test mail was sent during activation.
