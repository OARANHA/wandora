...
The SMTP credential exists only at `/opt/wandora/data/supabase/secrets/gotrue_smtp_pass` with `root:wandora-ops/0640` custody. It must not live in this repository, the Supabase `.env`, Compose text, browser-visible configuration or another service environment. ADR 0098 versions the reviewed Auth secret-file startup candidate and proves the exact one-service activation/rollback path. Because local Compose file secrets preserve host permissions and ignore `uid/gid/mode`, the wrapper reads the protected file during a minimal root startup window and immediately `exec su` drops GoTrue back to the image `supabase` UID/GID 1000. **Live GoTrue SMTP is still not activated** and remains on `supabase-mail:2500`; the next slice is the separately reviewed activation execution, still without invite/recovery/test mail.
