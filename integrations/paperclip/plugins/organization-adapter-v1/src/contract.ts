...
export function parseReconcileWebhook(input: {
  ...
  if (!companyId || catalogKey !== CATALOG_KEY || !timestamp || !signature) {
...
export function requireHmacSecret(secret: unknown): string {
  if (typeof secret !== 'string' || secret.length < 32 || secret.length > 8_192) {
    throw new Error('wandora_hmac_secret_invalid');
  }
  return secret;
}
